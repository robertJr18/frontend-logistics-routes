...
export default function Navbar({ title, backTo = "/portal" }: NavbarProps) {
  ...
  return (
    <header className="flex items-center gap-4 px-6 py-4 border-b border-white/10 bg-[hsl(var(--surface-1))]">
      <button onClick={() => navigate(backTo)} className="text-white/60 hover:text-white transition-colors" aria-label="Volver">
      ...
      <button onClick={handleLogout} className="text-white/60 hover:text-white transition-colors flex items-center gap-1.5 text-sm" aria-label="Cerrar sesión">
